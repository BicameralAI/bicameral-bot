    ...
    for context_path, item in evidence.items():
        ...
        if sha256 is not None:
            if not isinstance(sha256, str) or not SHA256_RE.match(sha256):
                errors.append(f"context_evidence.{context_path}.sha256 must be a 64-character lowercase SHA-256")
            elif context_root is not None:
                file_path = context_root / context_path
                if not file_path.is_file():
                    errors.append(f"context_evidence.{context_path}.sha256 cannot be verified; missing file: {file_path}")
                else:
...
def validate(data: dict[str, Any], require_context: set[str], context_root: Path | None = None) -> list[str]:
    errors: list[str] = []
    ...
    if not isinstance(loaded_context, list) or not all(isinstance(item, str) and item for item in loaded_context):
    ...
    if not isinstance(divergence, dict):
        ...
        if resolution == "promoted":
            ...
            if not isinstance(factory_pr, str) or not FACTORY_PR_RE.match(factory_pr):
                errors.append("promoted local setup divergence must include a bicameral-factory PR URL")
    ...
    if not isinstance(capture, dict):
        ...
        if factory_pr is not None and (not isinstance(factory_pr, str) or not FACTORY_PR_RE.match(factory_pr)):
            errors.append("end_of_session_capture.factory_pr must be null or a bicameral-factory PR URL")
...
def main() -> int:
    ...
    print(
        "Bicameral factory attestation OK: "
        + ", ".join(str(path) for path in attestation_paths)
    )
    return 0
