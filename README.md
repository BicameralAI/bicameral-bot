...
**Survey-informed defaults.** Bicameral's ADRs should be informed by public developer surveys and marketplace evidence, not only internal taste. Git can be the boring default for repo-centric teams while the architecture remains substrate-neutral. Local grounding should be useful before paid cloud; hosted graph intelligence should earn its place through cross-repo, historical, and conflict-analysis value. See [Public Developer Survey Implications](docs/research/public-developer-survey-implications.md).
